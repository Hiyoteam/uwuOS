...
def save(file,mode,text):
	file_=open(file,mode,"UTF-8-sig")
	file_.write(text)
	file_.close()
...
def send(text,nick=None,light=True):
	...
	if nick:
		ws.send(dumps({"cmd":"whisper","nick":nick,"text":text_}))

	else:
		ws.send(dumps({"cmd":"emote","text":text_}))
...
def chan(nick,state=False):
	...
	if data["push"]["wolf"] and not state and nick in data["msg"]:
		sleep(1)
		send(f"这时 [{nick}]{choice(general[9])}的遗言被发现了\n[{nick}]：[{data['msg'][nick]}]")
...
def var(name,nick):
	...
	if name=="botpass":
		send(f"botpass:Never Gonna Give You Up",nick,False)

	else:
		for uwu_,_uwu in list(list_.items()):
			if uwu_==name:
				print(f"{uwu_}:{_uwu}\n")
				send(f"{uwu_}:{_uwu}",nick,False)
...
# UNO 卡牌列表
paper={"first":["红","黄","蓝","绿"],"last":["禁","转","+2",*[str(_) for _ in range(9)][::2]],"sp":"变色"}
...
while True:
	...
	if "cmd" in wss:
		...
		if cmd=="chat":
			...
			if wss.get("trip") and wss["trip"]!="null":
				trip=wss["trip"]
				slices=text.split()
				pull(nick,trip)
				...
				if slices[0]=="|data":
					...
					if date_ in notlife[1][trip][2][1:]:
						...
						send(f"[{nick}]是今天第[{len(dker)}]个签到的\n今天是星期[{weekday}] 获得[{plus}]eb")
			...
			if f"@{botnick}" in text:
				...
				if wss.get("trip") in root+rootlist:
					try:
				...
				if any(data["push"].values()):
					...
					if slices[-1]=="truth":
						...
						send(f"[真心话]开始 请用`r`获取随机数")

					elif slices[-1]=="grass":
						data={**data,"temp":[],"core":False,"msg":[],"trip":[1]}
						send(f"开始清点[生草机]人数 请用`1`报数")
						...
						send(f"新的[村长]诞生了 {choice(general[10])}的夜晚要开始了")
							...
							while True:
								...
								for uwu in note:
									if uwu[1]==2:
										if uwu[3]:
											sleep(2)
											send(f"[{card[2][0]}]No.{str(randint(1,999)).zfill(3)}请睁眼")
											...
											for _ in range(60):
												...
												sleep(1)
												send(f"[{card[2][0]}][{uwu[0]}]在今晚{choice(general[1])} 突然猝死")

										else:
											sleep(randint(10,20))
											send(f"[{card[2][0]}]{choice(prophet[4])}")

									elif uwu[1]==3 and not data["werewolf"]["skip"]:
										if uwu[3]:
											sleep(2)
											send(f"[{card[3][0]}]们请睁眼")
											...
											for _ in range(120):
												...
												for _ in data["werewolf"]["wait"]:
													...
													sleep(1)
													send(f"[{card[3][0]}][{_}]在今晚{choice(general[1])} 突然猝死")

										else:
											sleep(randint(10,20))
											send(f"狼群中的狼{choice(werewolf[7])}")

									elif uwu[1]==4:
										if uwu[3]:
											sleep(2)
											send(f"[{card[4][0]}]No.{str(randint(1,999)).zfill(3)}请睁眼")
											...
											if med:
												...
												sleep(2)
												send(f"[{card[4][0]}]在研究能{choice(witch[0])}的药水")

										else:
											sleep(15)
											send(f"[{card[4][0]}]{choice(witch[4])}")
								...
								for _ in range(180):
									...
									for _ in data["day"]["wait"]:
										if data["core"][_][1]!=4:
											...
											sleep(2)
											send(f"[{card[data['core'][_][1]][0]}][{_}]在白天{choice(general[1])} 突然猝死")
				...
				if data["push"]["truth"]:
					if len(data["core"])<2:
						...
						sleep(1)
						send(f"由[{data['core'][data['num'].index(uwu_)]}]向[{data['core'][data['num'].index(_uwu)]}]提问")
						...
						def loop():
							...
							for uwu in range(len(temp)):
								...
								for _ in range(60 if uwu<3 else 120):
									...
									send(f"{temp[uwu]}卡住了？那就让[{botnick}]帮你写吧",temp[uwu])

							sleep(2)
							send("".join(data["msg"][:3])+" "+" ".join(data["msg"][3:]))
					...
					if len(data["temp"])<2:
						...
						send(f"初始牌是[{paper['first'][data['now'][0][0]]+paper['last'][data['now'][0][1]]}] 由[{data['temp'][0]}]先出")
							...
							while True:
								try:
									...
									for _ in range(60):
										...
										oops+=1
										send(f"Oops! [{now}]超时了 错过了自己的回合")

										sleep(1)

										if oops==10:
											send(f"回合被跳过10次了 {choice(['恼','悲','气','卑'])}")

										else:
											send(f"现在的牌还是[{paper['first'][data['now'][0][0]]+paper['last'][data['now'][0][1]]}] 轮到[{next_(now)}]")
				...
				if data["push"]["grass"] and data["core"]:
					...
					if nick in data["core"]:
						...
						send(f"[{nick}]没有参与本局游戏")

			elif text=="|over":
				for uwu in ["truth","guess"]:
					if data["push"][uwu]:
						send(f"Ahh! 游戏在连续进行了[{data['round']}]回后结束了")
						end(uwu)
				...
				try:
					if not data["push"]["truth"] and not data.get("core"):
						if nick in data["temp"]:
							send(f"[{nick}]已经在花名册中了 现有[{len(data['temp'])}]人")

						elif data["push"]["uno"] and len(data["temp"])>=8:
							send(f"已经到[UNO]的人数上限了")

						else:
							data["temp"].append(nick)

							if data["push"]["wolf"]:
								send(f"[{nick}]{choice(general[6])}了村子 村里有{len(data['temp'])}人了")

							else:
								send(f"[{nick}]成功加入 现有{len(data['temp'])}人")

							if wss.get("trip"):
								data["trip"].append(wss["trip"])
				...
				if data["push"]["truth"]:
					if nick in data["core"]:
						send(f"[{nick}]已经r出了[{data['num'][data['core'].index(nick)]}]")
				...
				if data["push"]["uno"] and nick in data.get("now"):
					...
					if len(data["core"][nick])==1:
						sleep(1)
						send(f"[{nick}]：{choice(emote)} UNO!!!")

					if not data["core"][nick]:
						sleep(1)
						send(f"Wow! [{nick}]出完了全部的牌 [{nick}]获胜")
						...
						send(f"{note} 现在的牌是[{paper['first'][data['now'][0][0]]+paper['last'][data['now'][0][1]]}]")
				...
				if data["push"]["guess"]:
					...
					try:
						...
						if num not in range(data["range"][0]+1,data["range"][1]):
							send(f"已经超出当前范围了 请在[{data['range'][0]}]~[{data['range'][1]}]之间")
							...
							if num==data["num"]:
								send(f"Wow! [{nick}]猜中了数字 {choice(['毫无疑问','的的确确','确实','果真'])}是[{num}]")
								...
								sleep(1)
								send(f"下轮游戏开始 范围在[{data['range'][0]}]~[{data['range'][1]}]间")

							else:
								note=["小","大"]
								note_=int(num>data["num"])
								data["range"][note_]=num
								send(f"[{nick}]猜{note[note_]}了 但把范围缩到了[{data['range'][0]}]~[{data['range'][1]}] {choice(['再来','再猜','继续','再接再厉'])}")

					except:
						send("这不是一个数字 请猜数字")

		elif cmd=="info" and wss.get("type")=="whisper" and "from" in wss:
			sleep(0.5)
			nick=str(wss["from"])
			text=wss["text"]
			trip=wss.get("trip",choice(root))
			uwu=wss.get("msg") if wss.get("msg") else text[text.find(":")+2:]
			msg=uwu.split("\n",1)
			slices=msg[-1].split()
			pull(nick,trip)
			...
			if msg[0]=="buy" and trip:
					...
					if name in notlife[0]:
						...
						if slices[2].isnumeric() and 0<=int(slices[2])<=1919810:
							...
							if notlife[1][trip][0][2]<note:
								...
								send(f"Wow! [{nick}]做出了[{name}]\n说明|[{slices[1]}] 价值|[{int(slices[2]) or 'Null'}]eb")

						else:
							send("价值只能为整数 且在0~1919810间",nick)

					else:
						send("物品名在1~12字间 说明在1~48字间",nick)

			elif msg[0]=="give" and trip:
				if len(slices)!=2:
					send(f"参数错误 应该是：`/w {botname} give(换行)物品名 识别码`",nick)
				...
				if len(slices)!=1:
					send(f"参数错误 应该是：`/w {botname} del(换行)物品名`",nick)
				...
				if not data["core"] and nick==data["owner"]:
					...
					if len(msg)<2 or not msg[1].strip():
						...
						send("遗言更新成功" if nick in data["msg"] else "遗言添加成功",nick)
					...
					if nick in data["day"]["wait"]:
						if uwu in data["state"][True]:
							...
							sleep(2)
							send(f"投票箱中多了一张{choice(general[4])}的纸片")

						elif uwu in data["state"][False]:
							send(f"你没必要{choice(general[7])}死者",nick)

						else:
							data["day"]["wait"].pop(data["day"]["wait"].index(nick))
							send(f"你{choice(general[5])} 放弃了投票",nick)

							sleep(2)
							send(f"有一张纸片{choice(general[8])}")
						...
						if data["now"][0]==2:
							if uwu==nick:
								...
								if uwu in data["state"][True]:
									if uwu in data["prophet"]:
										...
										sleep(2)
										send(f"不幸的[{card[2][0]}]{choice(prophet[0])}")
									...
									send(f"你{choice(general[0])}今晚放弃预言 睡个好觉",nick)

									sleep(2)
									send(f"[{card[2][0]}]睡得很{choice(prophet[3])}")
							...
							if uwu in data["state"][True]:
								...
								send(f"你{choice(general[0])}杀{note}",nick)

								for _ in data["werewolf"]["wait"]:
									sleep(1)
									send(f"你的同伴[{nick}]{choice(general[0])}杀{'你' if _==uwu else note}",_)
								...
								data["werewolf"]["wait"].pop(data["werewolf"]["wait"].index(nick))
								send(f"你{choice(general[0])}今晚做件好事 放过人类",nick)

								for _ in data["werewolf"]["wait"]:
									sleep(1)
									send(f"你的同伴[{nick}]{choice(general[3])}了",_)
							...
							if not data["werewolf"]["wait"]:
								...
								for _ in data["werewolf"]["core"]:
									sleep(1)
									send("今晚 你被杀死了" if _==uwu else f"今晚 {note}",_)

								sleep(2)

								if uwu in data["core"] and not data["core"][uwu][3]:
									send(f"[公告] {choice(general[2])} 只听见[{uwu}]的{choice(werewolf[0])}和几声狼嚎\n当[{choice(data['state'][True])}]跑去查看情况的时候 只发现[{uwu}]{choice(werewolf[1])}的尸体")

								else:
									send(f"能听见远处狼群{choice(werewolf[2])}的声音")
							...
							if uwu==nick:
								...
								send(f"你没必要{choice(general[7])}猝死的人",nick)
								...
								if uwu in data["witch"]["allow"]:
									...
									send(f"你{choice(general[0])}今晚放弃投药 等待机会",nick)

									sleep(2)
									send(f"[{card[4][0]}]笑得很{choice(witch[3])}")
